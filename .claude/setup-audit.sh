...
echo ""
read -rp "  Min changed lines [10]: " MIN_CHANGES
MIN_CHANGES="${MIN_CHANGES:-10}"

# ── Write .env ──
# Preserve existing non-audit variables if .env exists
EXISTING_VARS=""
if [[ -f "$ENV_FILE" ]]; then
  # Keep lines that don't start with ENABLE_CROSS_AUDIT or CROSS_AUDIT_
  EXISTING_VARS=$(grep -v '^\s*#' "$ENV_FILE" | grep -v '^ENABLE_CROSS_AUDIT' | grep -v '^CROSS_AUDIT_' | grep -v '^\s*$' || true)
fi

{
  echo "# Cross-LLM Audit Configuration"
  echo "# Generated by setup-audit.sh on $(date +%Y-%m-%d)"
  echo "# See Docs/CROSS-LLM-AUDIT.md for details."
  echo ""
  echo "ENABLE_CROSS_AUDIT=true"
  echo "CROSS_AUDIT_API_KEY=$API_KEY"
...
echo "╠══════════════════════════════════════════════════════╣"
echo "║                                                     ║"
echo "║  Config written to: .env (git-ignored)              ║"
