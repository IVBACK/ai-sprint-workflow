...
SRC_DIR="$ROOT/app"         # ← adjust to your source directory
TEST_DIR="$ROOT/tests"      # ← adjust to your test directory
EXT="py"                    # ← adjust: "cs", "ts", "py", "java", "go", "rs", "cpp"

# ── Modular adapter settings ──
CHECKS_DIR="$(cd "$(dirname "$0")" && pwd)/checks"  # ← beside sprint-audit.sh (Tools/checks/)
LANG_ADAPTER="python"       # ← set to adapter filename: "typescript-react", "python", etc.
...
if [[ -d "$SRC_DIR" ]]; then
  ...
  _ucount=$(echo "$_untracked" | grep -c . 2>/dev/null || true)
...
if [[ -f "$TRACKING_FILE" ]] && [[ -f "$ROADMAP_FILE" ]]; then
  ...
  # declare -A requires bash 4+ (macOS ships bash 3.2); skip sync check on older bash
  if [[ ${BASH_VERSINFO[0]} -lt 4 ]]; then
    echo "  SKIP  Roadmap↔TRACKING sync (requires bash 4+; current: bash ${BASH_VERSION})"
  else
...
if [[ -f "$TRACKING_FILE" ]]; then
  cl_entries=$(sed -n '/^## Change Log/,/^## [^C]/p' "$TRACKING_FILE" | grep -cE '^- ' 2>/dev/null || true)
  has_items=$(grep -cE "$ID_PATTERN.*(open|in_progress|fixed|verified)" "$TRACKING_FILE" 2>/dev/null || true)
...
if [[ -f "$TRACKING_FILE" ]]; then
  encounters=$(sed -n '/^## Failure Encounters/,/^## [^F]/p' "$TRACKING_FILE" | grep -cE '^\|[^-]' 2>/dev/null || true)
  encounters=$((encounters > 1 ? encounters - 1 : 0))  # subtract header row
  history=$(sed -n '/^## Failure Mode History/,/^## [^F]/p' "$TRACKING_FILE" | grep -cE '^\|[^-]' 2>/dev/null || true)
...
if [[ -f "$CLAUDE_FILE" ]]; then
  if grep -qE '## Last Checkpoint' "$CLAUDE_FILE" 2>/dev/null; then
    ...
    if [[ -z "$cp_content" ]]; then
      ...
      total=$((total + 1))
    elif echo "$cp_content" | grep -qE '\[YYYY-MM-DD\]|\[Sprint N|<!-- FILL'; then
...
# ═══════════════════════════════════════════════════════
# SECTION 17: CLAUDE.md Available Tools section
# Without this section, AI agents don't know sprint-tools
# exist and will manually edit TRACKING/Roadmap/CLAUDE.md
# instead of using atomic tool commands.
# ═══════════════════════════════════════════════════════

echo ""
echo "── AVAILABLE TOOLS ──"
if [[ -f "$CLAUDE_FILE" ]]; then
  if grep -qE '## Available Tools' "$CLAUDE_FILE" 2>/dev/null; then
    if grep -q 'sprint-tools item' "$CLAUDE_FILE" 2>/dev/null; then
      echo "  PASS  §Available Tools section present with sprint-tools reference"
    else
      echo "  WARN  §Available Tools section exists but missing sprint-tools commands"
      total=$((total + 1))
    fi
  else
    echo "  FAIL  §Available Tools section missing from CLAUDE.md — AI will not use sprint-tools"
    total=$((total + 1))
    blockers=$((blockers + 1))
  fi
else
...
# ═══════════════════════════════════════════════════════
# SECTION 18: CODING_GUARDRAILS.md TL;DR blocks
# Each numbered section should have a TL;DR line so agents
# can do progressive-disclosure reading (Index → TL;DR → full).
# ═══════════════════════════════════════════════════════

echo ""
echo "── GUARDRAILS TL;DR ──"
GUARDRAILS_FILE="$ROOT/Docs/CODING_GUARDRAILS.md"
if [[ -f "$GUARDRAILS_FILE" ]]; then
  # Count numbered sections (## 1. ... , ## 2. ... etc.)
  section_count=$(grep -cE '^## [0-9]+\.' "$GUARDRAILS_FILE" 2>/dev/null || true)
  if [[ $section_count -eq 0 ]]; then
    echo "  SKIP  (no numbered sections found — guardrails may not be populated yet)"
  else
    tldr_count=$(grep -ciE '^\*\*TL;DR' "$GUARDRAILS_FILE" 2>/dev/null || true)
    # Also match "> **TL;DR" blockquote format
    tldr_count=$((tldr_count + $(grep -cE '^>\s*\*\*TL;DR' "$GUARDRAILS_FILE" 2>/dev/null || true)))
    if [[ $tldr_count -ge $section_count ]]; then
      echo "  PASS  All $section_count sections have TL;DR ($tldr_count found)"
    else
      echo "  WARN  $tldr_count/$section_count sections have TL;DR — add missing ones for progressive disclosure"
      total=$((total + 1))
    fi
  fi
else
  echo "  SKIP  (CODING_GUARDRAILS.md not found)"
fi

# ═══════════════════════════════════════════════════════
# SECTION 19: Evidence confidence level audit
