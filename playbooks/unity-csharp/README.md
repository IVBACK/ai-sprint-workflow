...
The project accumulated guardrail rules over multiple sprints
and runs automated EditMode tests with audit scripts.
...
| 1 | Solo developer |
| 2 | Medium (5-8 items) |
| 3 | Yes — multi-phase roadmap exists |
...
| 9 | Yes — prior project experience encoded as guardrails |
| 10 | English (code, comments, commits) |
| 11 | Free-form (atomic preferred, monolithic acceptable for solo) |
| 12 | Data format, grid size, buffer type (see contracts below) |
...
- Grid size: NxNxN with ghost cells for boundary sampling
- Shader: #pragma use_dxc mandatory (platform requirement)
...
```
- Every compute dispatch and buffer operation: wrap with ProfilerMarker
...
├── Docs/
│   ├── CODING_GUARDRAILS.md               # Guardrail rules by category
│   ├── Workflow/                           # Split workflow docs (Entry Gate, Impl Loop, Close Gate, Sprint Close)
...
├── Tools/
│   ├── sprint-audit.sh                    # Heuristic scan
...
These work alongside Unity's Editor workflow. Run from the project root
terminal while the Editor handles builds and test execution.

## Key Learnings

1. **Guardrails from prior experience saved weeks.** Known issues were encoded as Day 0 rules. Zero repeat bugs from known issues.

2. **sprint-audit.sh dramatically reduced close gate context.** The script pre-filters mechanical issues, freeing AI context window for semantic analysis.
