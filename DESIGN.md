...
- **Workflow modes (Freestyle/Lite/Standard/Strict).** Not every project needs the same rigor. A hackathon needs zero ceremony; a team building a payment system needs full gates. Four presets control hook enforcement and gate depth: Freestyle (core safety only, ~0 min overhead), Lite (abbreviated gates, ~7 min), Standard (full workflow, ~15 min), Strict (no overrides, enforce-block, ~25 min). The mode is set via `WORKFLOW_MODE` in `hooks-config.sh` (Claude Code) or stated at session start (other agents). Individual hook flags can still override the preset — modes are defaults, not locks (except Strict, which prevents disabling hooks). Full comparison table: `Docs/Workflow/WORKFLOW-MODES.md`.
