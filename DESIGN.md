...
- **Workflow modes (Lite/Standard/Strict).** Not every project needs the same rigor. A solo prototype needs less ceremony than a team building a payment system. Three presets control hook enforcement and gate depth: Lite (5/10 hooks, abbreviated gate only, ~5 min overhead), Standard (10/10 hooks, full or abbreviated gate, ~15 min), Strict (10/10 hooks with overrides disabled, full gate always, ~25 min). The mode is set via `WORKFLOW_MODE` in `hooks-config.sh` (Claude Code) or stated at session start (other agents). Individual hook flags can still override the preset — modes are defaults, not locks (except Strict, which prevents disabling hooks).
